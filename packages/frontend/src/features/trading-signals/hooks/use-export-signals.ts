/**
 * Export Trading Signals Hook
 * 
 * React Query mutation hook for exporting trading signals to CSV.
 * Includes retry logic for transient errors.
 */

import { useMutation } from '@tanstack/react-query';
import { tradingSignalsService } from '@/infrastructure/api/services/trading-signals.service';
import type { ExportTradingSignalsQuery } from '@nexgent/shared';
...
/**
 * Hook to export trading signals to CSV
 * 
 * @returns React Query mutation for exporting signals with retry support
 * 
 * @example
 * ```tsx
...
export function useExportSignals() {
  ...
  const mutation = useMutation({
    mutationFn: async (query: ExportTradingSignalsQuery) => {
      // Create abort controller for this request
      const abortController = new AbortController();
      abortControllerRef.current = abortController;
      
      try {
        return await tradingSignalsService.exportTradingSignals(
          query,
    ...
    onError: (error: Error) => {
      ...
      toast({
        title: 'Export failed',
        description: error.message || 'Failed to export trading signals. Please try again.',
        variant: 'destructive',
      });
    },
  });
